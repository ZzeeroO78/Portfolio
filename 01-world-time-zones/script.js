...
async function updateWeather() {
    ...
    const weatherPromises = cards.map(async (card) => {
        ...
        if (realWeather) {
            ...
            const icon = getWeatherIcon(realWeather.weatherCode, realWeather.isDay);
            const season = getSeason(city.timezone);
            
            // Update temperature
            const tempSpan = card.querySelector('.weather-temp .temp-value');
            if (tempSpan) {
                tempSpan.textContent = tempDisplay;
            }
            const tempIcon = card.querySelector('.weather-temp .weather-icon');
            if (tempIcon) {
            ...
            if (humiditySpan) {
                humiditySpan.textContent = `${realWeather.humidity}% Humidity`;
            }
            
            // Update season
            const seasonIcons = {'winter': '❄️', 'spring': '🌸', 'summer': '🌞', 'autumn': '🍂'};
            const seasonSpan = card.querySelector('.weather-season .season-value');
            const seasonIcon = card.querySelector('.weather-season .weather-icon');
            if (seasonSpan) {
                seasonSpan.textContent = season.charAt(0).toUpperCase() + season.slice(1);
            }
            if (seasonIcon) {
                seasonIcon.textContent = seasonIcons[season] || '📅';
            }
        } else {
            ...
            const tempSpan = card.querySelector('.weather-temp .temp-value');
            if (tempSpan) {
                tempSpan.textContent = fallback.temp;
            }
        }
    });
...
function init() {
    ...
    // CRITICAL: Always ensure cities has content
    ...
    // Immediately update weather after cards are rendered
    updateWeather();
...
async function fetchRealWeather(cityName) {
    ...
    if (!coords) {
        console.log(`No API coords for: ${cityName}`);
        return null;
    }
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`API error for ${cityName}: ${response.status}`);
            return null;
        }
        ...
        if (data && data.current) {
            ...
            console.log(`✓ ${cityName}: ${result.temp}°C`);
            return result;
        }
        console.error(`No data for ${cityName}`);
        return null;
    } catch (error) {
        console.error(`Fetch error for ${cityName}: ${error.message}`);
        return null;
    }
}
...
function renderCities() {
    ...
    if (filteredCities.length === 0) {
        grid.innerHTML = `<div class="empty-state"><h2>📍 No cities</h2></div>`;
        return;
    }
...
function loadCitiesFromStorage() {
    ...
    if (stored) {
        try {
            ...
            console.log('Storage validation: reset cities to default');
        }
    }
    cities = [];
}
